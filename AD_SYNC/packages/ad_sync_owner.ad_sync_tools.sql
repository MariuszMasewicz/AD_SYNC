...
CREATE OR REPLACE PACKAGE BODY AD_SYNC_OWNER.AD_SYNC_TOOLS IS
  ...
  BEGIN
    BEGIN
      SELECT
        PARAMETER_VALUE INTO V_RET
      FROM
        AD_SYNC_OWNER.AD_SYNC_PARAMETERS P
      WHERE
        P.PARAMETER_NAME = P_PARAM_NAME;
    EXCEPTION
      WHEN OTHERS THEN
        V_RET := 'null';
        AD_SYNC_LOG.WRITE_ERROR($$PLSQL_UNIT
                                || '->get_param_value: p_param_name: '
                                || P_PARAM_NAME, SQLCODE, SQLERRM);
        RAISE;
    END;
 ...
 /*password, chars => 12, uppercase => 1, lowercase => 1,
  ...
  BEGIN
    SELECT
      LISTAGG(LETTER, NULL) WITHIN GROUP (ORDER BY DBMS_RANDOM.VALUE) INTO PASSWORD
    FROM
      (
        SELECT
          SUBSTR( DIGITS, FLOOR(DBMS_RANDOM.VALUE(1, LENGTH(DIGITS) + 1)), 1 )   AS LETTER
        FROM
          DUAL
        CONNECT BY
          LEVEL <= NO_OF_DIGITS
        ...
        ALL
        SELECT
          SUBSTR( LOWER, FLOOR(DBMS_RANDOM.VALUE(1, LENGTH(LOWER) + 1)), 1 )     AS LETTER
        FROM
          DUAL
        CONNECT BY
          LEVEL <= NO_OF_LOWER
        ...
        ALL
        SELECT
          SUBSTR( UPPER, FLOOR(DBMS_RANDOM.VALUE(1, LENGTH(UPPER) + 1)), 1 )     AS LETTER
        FROM
          DUAL
        CONNECT BY
          LEVEL <= NO_OF_UPPER
        ...
        ALL
        SELECT
          SUBSTR( SPECIAL, FLOOR(DBMS_RANDOM.VALUE(1, LENGTH(SPECIAL) + 1)), 1 ) AS LETTER
        FROM
          DUAL
        CONNECT BY
