...
CREATE OR REPLACE PACKAGE BODY AD_SYNC_OWNER.AD_SYNC_PROCESS_USERS AS
  ...
  BEGIN
    IF P_PROCESS_RUN IS NOT NULL THEN
      ...
      FOR I IN (
        SELECT AU.ID
              ,AU.USERNAME
          FROM AD_SYNC_OWNER.AD_SYNC_USERS AU
      ...
      COMMIT;
      AD_SYNC_LOG.WRITE_INFO(
        $$PLSQL_UNIT || '->drop_users  finished'
       ,SQLCODE
       ,SQLERRM
       ,P_PROCESS_RUN
      );
  ...
  BEGIN
    IF P_PROCESS_RUN IS NOT NULL THEN
      ...
      FOR I IN (
        ...
        EXECUTE IMMEDIATE V_STMT;
        UPDATE AD_SYNC_OWNER.AD_SYNC_USERS
           SET STATUS = 23 -- password expired
           ,PROCESS_TIMESTAMP = CURRENT_TIMESTAMP
        ,LOAD_ID = P_LOAD_ID
         WHERE ID = I.ID;
      END LOOP;
      ...
      COMMIT;
      AD_SYNC_LOG.WRITE_INFO(
        $$PLSQL_UNIT || '->expire_password  finished'
       ,SQLCODE
       ,SQLERRM
       ,P_PROCESS_RUN
      );
    ...
    IF P_PROCESS_RUN IS NOT NULL THEN
      ...
      FOR I IN (
        SELECT AU.ID
              ...
              ,NVL(
          AU.PASSWORD
         ,AD_SYNC_OWNER.AD_SYNC_TOOLS.GENERATE_PASSWORD
        ) AS PASSWORD
          FROM AD_SYNC_OWNER.AD_SYNC_USERS AU
          JOIN AD_SYNC_OWNER.AD_SYNC_MANAGED_USERS U
        ON ( AU.USERNAME = U.USERNAME )
        ...
        IF AD_SYNC_OWNER.AD_SYNC_TOOLS.GET_PARAM_VALUE('STORE_USER_PASSWORD_IN_TABLE') = '1' THEN
          UPDATE AD_SYNC_OWNER.AD_SYNC_USERS
             SET STATUS = 24 -- password changed
             ,PROCESS_TIMESTAMP = CURRENT_TIMESTAMP
          ,PASSWORD = I.PASSWORD
          ,LOAD_ID = P_LOAD_ID
           WHERE ID = I.ID;
 ...
 --, password = i.password
        ...
        IF AD_SYNC_OWNER.AD_SYNC_TOOLS.GET_PARAM_VALUE('STORE_USER_PASSWORD_IN_FILE') = '1' THEN
          ...
          UPDATE AD_SYNC_OWNER.AD_SYNC_USERS
             SET PASSWORD_FILE = V_FILE_NAME,LOAD_ID = P_LOAD_ID
           WHERE ID = I.ID;
          V_FILE      := UTL_FILE.FOPEN(
            AD_SYNC_OWNER.AD_SYNC_TOOLS.GET_PARAM_VALUE('STORE_USER_PASSWORD_IN_FILE_DIRECTORY')
           ,V_FILE_NAME
           ,'w'
          );
          UTL_FILE.PUT(
      ...
      COMMIT;
      AD_SYNC_LOG.WRITE_INFO(
        $$PLSQL_UNIT || '->change_password finished'
       ,SQLCODE
       ,SQLERRM
       ,P_PROCESS_RUN
      );
  ...
  BEGIN
    IF P_PROCESS_RUN IS NOT NULL THEN
      ...
      COMMIT;
      AD_SYNC_LOG.WRITE_INFO(
        $$PLSQL_UNIT || '->unlock_users finished'
       ,SQLCODE
       ,SQLERRM
       ,P_PROCESS_RUN
      );
  ...
  BEGIN
    IF P_PROCESS_RUN IS NOT NULL THEN
      ...
      COMMIT;
      AD_SYNC_LOG.WRITE_INFO(
        $$PLSQL_UNIT || '->lock_users finished'
       ,SQLCODE
       ,SQLERRM
       ,P_PROCESS_RUN
      );
 ...
 /* existing users*/
      UPDATE AD_SYNC_OWNER.AD_SYNC_USERS
         SET STATUS = 28 -- user exists in database
         ,PROCESS_TIMESTAMP = CURRENT_TIMESTAMP
      ,LOAD_ID = P_LOAD_ID
       WHERE USERNAME IN (
        SELECT USERNAME
          FROM AD_SYNC_OWNER.AD_SYNC_MANAGED_USERS
      )
         AND STATUS = 1
 ...
 /* bad prefix*/
      UPDATE AD_SYNC_OWNER.AD_SYNC_USERS
         SET STATUS = 27 -- invalid user prefix
         ,PROCESS_TIMESTAMP = CURRENT_TIMESTAMP
      ,LOAD_ID = P_LOAD_ID
       WHERE USERNAME NOT LIKE AD_SYNC_OWNER.AD_SYNC_TOOLS.GET_PARAM_VALUE('USERNAME_PREFIX')
                               || '%'
         AND STATUS = 1
         AND CREATED_TIMESTAMP BETWEEN P_START_TIMESTAMP AND P_END_TIMESTAMP;
      V_NUMBER_OF_EXISTING := SQL%ROWCOUNT;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      AD_SYNC_LOG.WRITE_ERROR(
        $$PLSQL_UNIT || '->mark_existing_users'
       ,SQLCODE
       ,SQLERRM
      );
      RAISE;
