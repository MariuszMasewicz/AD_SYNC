...
CREATE OR REPLACE PACKAGE AD_SYNC_OWNER.AD_SYNC_PROCESS_GROUPS
        ...
        IF P_PROCESS_RUN IS NOT NULL THEN
            ...
            FOR I IN (
                SELECT
                    G.GROUPNAME
                FROM
                    AD_SYNC_OWNER.AD_SYNC_GROUPS         AG
...
;
                ...
                AD_SYNC_LOG.WRITE_INFO($$PLSQL_UNIT
                                       ...
                                       || I.GROUPNAME, SQLCODE, SQLERRM, P_PROCESS_RUN);
            ...
            --        COMMIT;
            AD_SYNC_LOG.WRITE_INFO($$PLSQL_UNIT
                                   || '->drop_gropus_not_exist_in_load  finished', SQLCODE, SQLERRM, P_PROCESS_RUN);
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            AD_SYNC_LOG.WRITE_ERROR($$PLSQL_UNIT
                                    || '->drop_gropus_not_exist_in_load', SQLCODE, SQLERRM);
            RAISE;
...
,
            ...
            COMMIT;
            AD_SYNC_LOG.WRITE_INFO($$PLSQL_UNIT
                                   || '->drop_gropus  finished', SQLCODE, SQLERRM, P_PROCESS_RUN);
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            AD_SYNC_LOG.WRITE_ERROR($$PLSQL_UNIT
                                    || '->drop_gropus', SQLCODE, SQLERRM);
            RAISE;
            ...
            WHERE
                GROUPNAME IN (
                    SELECT
                        GROUPNAME
                    FROM
                        AD_SYNC_OWNER.AD_SYNC_MANAGED_GROUPS
                )
                AND STATUS = 1
...
,
            ...
            COMMIT;
            AD_SYNC_LOG.WRITE_INFO($$PLSQL_UNIT
                                   || '->add_groups finished', SQLCODE, SQLERRM, P_PROCESS_RUN);
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            AD_SYNC_LOG.WRITE_ERROR($$PLSQL_UNIT
